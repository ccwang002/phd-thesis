...
Depending on the use cases, the challenges for ID mappability, also known as the cross-reference, can be categorized as below:
\begin{itemize}
...
Moreover, the mappability of two IDs may depend on the user purpose.
In some cases, users just want the the ID mapping that are biologically similar.
In other cases, the sequence identity is required for the ID mapping.
...
\begin{table}[tbp]
    \centering
    \caption{Comparison of commonly used annotations. The bolded part of the identifiers denote their versioning schemes.}
