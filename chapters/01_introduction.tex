...
\section{Glioblastoma (GBM)}
Glioblastoma (GBM) is the most common primary malignant brain tumor with an incidence of 3.22 per 100,000 persons in the United States, a median survival of less than 2 years from diagnosis \cite{delgado-lopezpd_corrales-garciaem:SurvivalGlioblastoma2016,ostromqt_barnholtz-sloanjs:CBTRUSStatistical2019,ostromqt_barnholtz-sloanjs:CBTRUSStatistical2020} and limited treatment options \cite{limm_wellerm:CurrentState2018,reifenbergerg_wellerm:AdvancesMolecular2017}.
TCGA surveyed 206 GBMs with genomic and transcriptomic profiling \cite{tcga_network:GBM2008}, extending the subclassifications developed by Aldape and colleagues \cite{phillipshs_aldapek:MolecularSubclasses2006,verhaakrgw_hayesdn:IntegratedGenomic2010}.
The current view is that IDH-wild type GBMs fall into three distinct subclasses: proneural, classical, and mesenchymal, based on genomic alterations and gene expression signatures \cite{tcga_network:GBM2008,verhaakrgw_hayesdn:IntegratedGenomic2010,brennancw_chinl:GBM2013,ceccarellim_verhaakrgw:MolecularProfiling2016,noushmehrh_aldapek:IdentificationCpG2010,wangq_verhaakrgw:TumorEvolution2017}.
The tumor microenvironment may also play an important role in GBM pathogenesis, where tumor-associated macrophages (TAMs) comprise the majority of the immune population \cite{charlesna_kettenmannh:BrainTumor2012,hambardzumyand_kettenmannh:RoleMicroglia2016,quaildf_joyceja:MicroenvironmentalLandscape2017}.
TAMs originate from tissue-resident microglia and bone marrow-derived macrophages (BMDMs), with evidence suggesting that they facilitate tumor proliferation, survival, and migration \cite{hambardzumyand_kettenmannh:RoleMicroglia2016}.
M2-type macrophages, typically thought of as immunosuppressive, dominate and may serve as a microenvironmental advantage to the tumor \cite{kennedybc_andersonrce:TumorAssociatedMacrophages2013}.

Despite extensive molecular and immunological characterization of GBM, surgical resection followed by concurrent chemotherapy and radiotherapy remain the standard of care \cite{stuppr_mirimanoffro:RadiotherapyConcomitant2005,hegime_stuppr:MGMTGene2005,perryjr_trialinvestigators:ShortCourseRadiation2017}, and targeted therapies have been disappointing so far.
Several promising immunotherapies have been proposed, including checkpoint inhibitors, vaccines, Chimeric antigen receptor (CAR) T-cell therapy, and viral therapy, though none have yet demonstrated therapeutic efficacy in phase 3 clinical trials \cite{limm_wellerm:CurrentState2018,reifenbergerg_wellerm:AdvancesMolecular2017,mcgranahant_nagpals:CurrentState2019}.
Importantly, the current state of the art method treats all GBM patients uniformly upon diagnosis. No treatment has been found to work in a pre-specified subset of patients based on transcriptomic subtypes.
Hence, there is an urgent need for a more nuanced understanding of tumor cells beyond gene expression, and the tumor microenvironment in a manner that is predictive of precision therapy efficacy.
...
As a result, GBM is selected as one of the cancer type to be studied in CPTAC using proteogenomic multi-omics assays.
\fref{fig:intro-cptac-gbm-study-design} shows the CPTAC GBM experiment design.
In the GBM discovery cohort, GBM pre-treatment primary tumor segments are collected from the patients and cryopulverized into powder, later distributed to different downstream multi-omics assays.
For selected tumor samples, single cell sequencing experiments were conducted to capture the gene expression and chromatin accessibility at single cell level.



\section{A toolbox for multi-omics analysis and proteogenomic characterization of glioblastoma}
My thesis work contributes to precision oncology with two directions: expanding the computational toolbox for multi-omics analysis and building insights into disease biology and treatment of glioblastoma. In \Cref{chap:mut-pipeline-qc}, we profiled the somatic mutation calling pipeline, one of the the data harmonization pipelines on GDC to ensure they can be confidently used on multiple large-scale studies and identified the technical artifacts introduced by different pipelines. In \Cref{chap:ptmcosmos}, we built PTMcosmos, a web portal to collect PTM annotations and CPTAC experimental data to allow users to study the function of PTM in cancer. Finally, in \Cref{chap:cptac-gbm-discov}, we performed proteogenomic characterization of GBM as part of the CPTAC studies and provided potential patient stratifications and additional therapeutic avenues.
